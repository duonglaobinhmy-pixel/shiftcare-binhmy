...
export default function Layout(){
  ...
  const [collapsed,setCollapsed]=useState(()=>{try{return localStorage.getItem('shiftcare_sidebar_collapsed')==='1'}catch{return false}});
  ...
  return <div className={`app-shell ${collapsed?'sidebar-collapsed':''}`}>
    <aside className="app-sidebar">
      ...
      <nav className="sidebar-nav">
        {visibleMenus.map(item=><NavLink key={item.to} to={item.to} end={item.end} title={collapsed?item.label:undefined} className={({isActive})=>isActive?'active':''}>
