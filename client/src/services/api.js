...
async function request(path,options={}){
  ...
  try{
    const res=await fetch(`${API}${path}`,{...options,headers,signal:options.signal||controller.signal,body:options.body&&!(options.body instanceof FormData)&&typeof options.body!=='string'?JSON.stringify(options.body):options.body});
    const data=await res.json().catch(()=>({message:`HTTP ${res.status}`}));
    if(!res.ok){const detail=typeof data?.error==='string'?data.error:data?.error?.message;throw new Error(data?.message||detail||`HTTP ${res.status}`)}
    return data;
  }catch(e){
    if(e?.name==='AbortError')throw new Error('API phản hồi quá lâu. Hãy kiểm tra backend/server rồi thử lại.');
    throw e;
...
export const api={
 ...
 aiChat:(input)=>{const message=typeof input==='string'?input:String(input?.message||'');return request('/ai/chat',{method:'POST',body:{message}})},cleanTranscript:(text)=>request('/ai/clean-transcript',{method:'POST',body:{text}}),transcribeAudio:(audioBase64,mimeType)=>request('/ai/transcribe-audio',{method:'POST',body:{audioBase64,mimeType}})
};
