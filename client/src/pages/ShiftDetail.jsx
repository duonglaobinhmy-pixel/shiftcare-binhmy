...
const freshForm=()=>({clientRequestId:requestId(),entryMode:'QUICK',category:'HEALTH',categoryCodes:['HEALTH'],eventType:'OBSERVATION',eventCodes:['OBSERVATION'],residentStatus:'IN_FACILITY',priority:'MEDIUM',occurredAt:new Date().toISOString().slice(0,16),content:'',intervention:'',requiresHandover:false,followUp:'',vitalConcern:false,bpSys:'',bpDia:'',pulse:'',spo2:'',temperature:'',respiratoryRate:'',bloodGlucose:'',insulinGiven:false,insulinOrderId:'',insulinDose:'',urgentConfirmed:false,urgentSymptoms:'',bowelStatus:'NORMAL',urineStatus:'NORMAL',urineDetail:'',note:'',woundImages:[]});
...
export default function ShiftDetail(){
  ...
  useEffect(()=>{if(!d)return;const rid=routeResidentId||searchParams.get('residentId'),openForm=!!routeResidentId||searchParams.get('quick')==='1'||searchParams.has('mode');if(rid&&openForm){const r=d.residents.find(x=>String(x.residentId)===String(rid));if(r){setSel(r);setForm({...freshForm(),entryMode:searchParams.get('mode')==='full'?'FULL':'QUICK'})}}},[d,routeResidentId,searchParams]);
  ...
  function closeEntry(){
    ...
    setSel(null);
    setErr('');
    if(routeResidentId)navigate(`/shifts/${id}`,{replace:true});
  }
  ...
  async function save(){if(!sel||!review)return;const closeAfter=review.closeAfter;setSaveBusy(true);setErr('');try{
    const hasVitals=['bpSys','bpDia','pulse','spo2','temperature','respiratoryRate','bloodGlucose'].some(field=>form[field]!=='')||form.vitalConcern;
    const r=await api.change({clientRequestId:form.clientRequestId,shiftId:id,residentId:sel.residentId,residentName:sel.fullName,areaId:sel.areaId,areaName:sel.areaName,roomName:sel.roomName,bedName:sel.bedName,image:sel.image,category:form.categoryCodes[0],categoryCodes:form.categoryCodes,eventType:'OBSERVATION',eventCodes:['OBSERVATION'],residentStatus:form.residentStatus,priority:redAlerts.length?'HIGH':form.priority,occurredAt:form.occurredAt?new Date(form.occurredAt).toISOString():new Date().toISOString(),content:form.content.trim(),intervention:form.intervention.trim(),notifiedTo:'',requiresHandover:form.requiresHandover,followUp:form.followUp.trim(),vitals:hasVitals?{bpSys:form.bpSys,bpDia:form.bpDia,pulse:form.pulse,spo2:form.spo2,temperature:form.temperature,respiratoryRate:form.respiratoryRate,bloodGlucose:form.bloodGlucose,concern:form.vitalConcern||redAlerts.length>0,alertLevel:redAlerts.length?'RED':yellowAlerts.length?'YELLOW':'NORMAL',alerts:vitalAlerts,urgent:redAlerts.length?{confirmed:form.urgentConfirmed,action:form.intervention.trim(),symptoms:form.urgentSymptoms.trim()}:null}:null,insulin:form.insulinGiven?{given:true,dose:form.insulinDose,unit:'IU'}:null,woundImages:form.entryMode==='FULL'?form.woundImages.map(({dataUrl,sizeBytes,width,height})=>({dataUrl,sizeBytes,width,height})):[]},form.woundImages.length?{skipQueue:true}:undefined);
    ...
    if(closeAfter){closeEntry()}else{const keep={...freshForm(),entryMode:form.entryMode,category:form.category,priority:form.priority};setForm(keep)}
  ...
  return <section className={routeResidentId?'entry-route-page':''}>
    ...
    {sel&&<div className="modal"><div className="modal-card wide entry-modal">
      <div className="modal-head"><div><h2>Ghi nhận chăm sóc — {sel.fullName}</h2><p>{sel.areaName} • {sel.roomName} • {sel.bedName}</p></div><button type="button" className="secondary" onClick={closeEntry}>{routeResidentId?'← Quay về ca trực':'Đóng'}</button></div>
