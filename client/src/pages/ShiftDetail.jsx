...
export default function ShiftDetail(){
  ...
  function prepareSave(closeAfter=false){
    ...
    if(!form.eventCodes.length){setErr('Chọn ít nhất một nội dung ghi nhận.');return}
    ...
    if(form.entryMode==='FULL'&&form.insulinGiven){
    ...
    {sel&&<div className="modal"><div className="modal-card wide entry-modal">
      ...
      <div className="modal-body entry-modal-body">
        ...
        <div className="entry-layout"><div className="entry-form">
          ...
          <fieldset className="vitals-fieldset"><legend>Nội dung ghi nhận (chọn nhiều, lưu một lần)</legend><div className="form-grid entry-grid">{eventTypes.map(([code,label])=><label className="check" key={code}><input type="checkbox" checked={form.eventCodes.includes(code)} onChange={e=>setForm(current=>{const eventCodes=e.target.checked?[...current.eventCodes,code]:current.eventCodes.filter(x=>x!==code);return {...current,eventCodes,category:eventCodes.includes('FALL')?'INCIDENT':current.category}})}/>{label}</label>)}</div></fieldset>
