...
export async function middlewareSchemaReady() {
  ...
  const value=Boolean(r.rows[0]?.shifts && r.rows[0]?.residents_ref && r.rows[0]?.care_event_catalog && r.rows[0]?.care_record_events && r.rows[0]?.resident_status);
...
export async function databaseHealth() {
  ...
  try {
    const r = await getPool().query(`SELECT NOW() AS now, to_regclass('public.shifts') AS shifts, to_regclass('public.bcare_residents_ref') AS residents_ref, to_regclass('public.care_event_catalog') AS care_event_catalog, to_regclass('public.care_record_events') AS care_record_events, EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='care_records' AND column_name='resident_status') AS resident_status`);
    const ready=Boolean(r.rows[0]?.shifts && r.rows[0]?.residents_ref && r.rows[0]?.care_event_catalog && r.rows[0]?.care_record_events && r.rows[0]?.resident_status);
    schemaCache={value:ready,expiresAt:Date.now()+30_000};
    return {mode:'POSTGRESQL_MIDDLEWARE',ok:true,now:r.rows[0]?.now,middlewareSchemaReady:ready,careEventsReady:!!r.rows[0]?.care_event_catalog&&!!r.rows[0]?.care_record_events,residentStatusReady:!!r.rows[0]?.resident_status};
  } catch (error) {
