...
function canAccessShift(user,shift){
  return visibleByScope(user,shift);
}
...
async function createShift(user,body,{auto=false}={}){
  ...
  if(assignedStaff.length<2||assignedStaff.length!==requestedIds.length)throw new Error('Mỗi ca phải chọn tối thiểu 2 nhân sự đang hoạt động thuộc đúng cơ sở.');
...
router.get('/shifts/:id/glucose-schedule',allowPermission('SHIFT.VIEW'),async(req,res)=>{
  ...
  if(!shift||!canAccessShift(req.user,shift))return res.status(404).json({success:false,message:'Không tìm thấy ca'});
  const residents=store.shiftResidents.filter(x=>x.shiftId===shift.id),ids=residents.map(x=>String(x.residentId));
  if(!ids.length)return res.json({success:true,data:[]});
  const db=getPool();if(!db)return res.status(503).json({success:false,message:'Lịch đo đường huyết cần PostgreSQL.'});
...
router.put('/shifts/:id/glucose-schedule/:residentId',allowPermission('CARE.UPDATE'),async(req,res)=>{
  ...
  if(!shift||!canAccessShift(req.user,shift)||!resident)return res.status(404).json({success:false,message:'Không tìm thấy NCT trong ca'});
  if(shift.status!=='OPEN')return res.status(423).json({success:false,message:'Ca đã bàn giao, không thể sửa lịch.'});
  if(typeof req.body?.enabled!=='boolean')return res.status(422).json({success:false,message:'Thiếu trạng thái lịch theo dõi.'});
  const intervalDays=Number(req.body.intervalDays||7);
  if(!Number.isInteger(intervalDays)||intervalDays<1||intervalDays>365)return res.status(422).json({success:false,message:'Chu kỳ phải từ 1 đến 365 ngày.'});
  const db=getPool();if(!db)return res.status(503).json({success:false,message:'Lịch đo đường huyết cần PostgreSQL.'});
...
router.post('/change-logs',allowPermission('CARE.CREATE'),async(req,res)=>{
  ...
  if(!b.residentId||!b.category)return res.status(400).json({success:false,message:'Thiếu NCT hoặc nhóm ghi nhận'});
...
router.patch('/change-logs/:id',allowPermission('CARE.UPDATE'),async(req,res)=>{
  ...
  if(!old)return res.status(404).json({success:false,message:'Không tìm thấy bản ghi'});
  const shift=s.shifts.find(x=>x.id===old.shiftId);
  if(!shift||!canAccessShift(req.user,shift)||!visibleByScope(req.user,old))return res.status(403).json({success:false,message:'Không có quyền sửa bản ghi này'});
  if(shift.status!=='OPEN')return res.status(423).json({success:false,message:'Ca đã bàn giao, không thể sửa ghi nhận.'});
  if(req.user.role==='CAREGIVER'&&old.createdBy!==req.user.sub)return res.status(403).json({success:false,message:'Không có quyền sửa bản ghi này'});
  const categoryCodes=b.categoryCodes===undefined?old.categoryCodes:(Array.isArray(b.categoryCodes)?[...new Set(b.categoryCodes)]:[]);
  if(!categoryCodes.length||categoryCodes.some(code=>!careCategories.has(code)))return res.status(422).json({success:false,message:'Chọn ít nhất một nhóm ghi nhận hợp lệ.'});
  ...
  await updateStore(store=>{const row=store.changeLogs.find(x=>x.id===old.id);if(!row)throw new Error('Bản ghi không còn tồn tại.');Object.assign(row,next)});
