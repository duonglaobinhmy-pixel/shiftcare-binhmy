...
async function loadRosterForShift(shift){
  ...
  if(shift.areaId)items=items.filter(x=>!x.areaId||x.areaId===shift.areaId);
  await updateStore(store=>{
    const existing=new Set(store.shiftResidents.filter(x=>x.shiftId===shift.id).map(x=>x.residentId));
    for(const r of items){if(existing.has(r.id))continue;store.shiftResidents.push({id:uuid(),shiftId:shift.id,residentId:r.id,code:r.code,fullName:r.fullName,branchId:r.branchId||shift.branchId,branchName:r.branchName||shift.branchName,areaId:r.areaId||null,areaName:r.areaName||'',roomId:r.roomId||null,roomName:r.roomName||'',bedName:r.bedName||'',image:r.image||'',derivedStatus:'NO_RECORDED_CHANGE'})}
  });
  return items.length;
}
async function createShift(user,body,{auto=false}={}){
  ...
  const shift={id:uuid(),shiftDate:body.shiftDate||todayVN(),shiftType,status:'OPEN',branchId,branchName,areaId:body.areaId||null,areaName:body.areaName||'',roomId:body.roomId||null,assignedStaffIds:assignedStaff.map(x=>x.id),assignedStaff,assignedStaffNames:assignedStaff.map(x=>x.fullName),assignedStaffId:primaryRecorder.id,assignedStaffName:primaryRecorder.fullName,primaryRecorderId:primaryRecorder.id,primaryRecorderName:primaryRecorder.fullName,primaryRecorderCode:primaryRecorder.employeeCode,autoCreated:auto,createdBy:user.sub,createdAt:new Date().toISOString()};
  await updateStore(store=>store.shifts.push(shift));
  const residentCount=await loadRosterForShift(shift);
  await audit(user,auto?'SHIFT_AUTO_CREATE':'SHIFT_CREATE','shift',shift.id,{residentCount,areaId:shift.areaId});
...
router.get('/dashboard',allowPermission('DASHBOARD.VIEW'),async(req,res)=>{
  const date=req.query.date||todayVN(),s=await getStore();
  const shifts=s.shifts.filter(x=>x.shiftDate===date&&visibleByScope(req.user,x));const ids=new Set(shifts.map(x=>x.id));
  const residents=s.shiftResidents.filter(x=>ids.has(x.shiftId)&&visibleByScope(req.user,x));const changes=s.changeLogs.filter(x=>!x.deleted&&ids.has(x.shiftId)&&visibleByScope(req.user,x)).map(withAttention);const handovers=s.handovers.filter(x=>ids.has(x.shiftId));
  const allVisibleChanges=s.changeLogs.filter(x=>!x.deleted&&visibleByScope(req.user,x)).map(withAttention);const outstanding=allVisibleChanges.filter(x=>x.attentionLevel&&x.attentionStatus==='OPEN').sort((a,b)=>(a.attentionLevel===b.attentionLevel?String(b.occurredAt||b.createdAt).localeCompare(String(a.occurredAt||a.createdAt)):a.attentionLevel==='RED'?-1:1));
  const byCategory=changes.reduce((a,x)=>(a[x.category]=(a[x.category]||0)+1,a),{});const shiftStatus=Object.entries(shifts.reduce((a,x)=>(a[x.status]=(a[x.status]||0)+1,a),{})).map(([status,count])=>({status,count}));
  ...
  res.json({success:true,data:{date,todayShifts:shifts.length,uniqueResidents:new Set(residents.map(x=>x.residentId)).size,changeLogs:changes.length,openRed:outstanding.filter(x=>x.attentionLevel==='RED').length,openYellow:outstanding.filter(x=>x.attentionLevel==='YELLOW').length,resolvedToday:changes.filter(x=>x.attentionLevel&&x.attentionStatus==='RESOLVED').length,requiresHandover:changes.filter(x=>x.requiresHandover).length,pendingReceive:handovers.filter(x=>x.confirmedAt&&!x.receivedAt).length,byCategory,shiftStatus,alerts}});
});

router.get('/shifts',allowPermission('SHIFT.VIEW'),async(req,res)=>{
  const s=await getStore();let rows=s.shifts.filter(x=>canAccessShift(req.user,x));rows=rows.sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))).map(x=>({...x,residentCount:shiftResidentCount(s,x.id)}));res.json({success:true,data:rows});
});
router.get('/shifts/staff-options',allowPermission('SHIFT.CREATE','SHIFT.UPDATE'),async(req,res)=>{const requestedBranchId=String(req.query.branchId||''),branchId=req.user.role==='ADMIN'?requestedBranchId:String(req.user.branchId||'');if(!branchId)return res.status(400).json({success:false,message:'Thiếu cơ sở.'});const store=await getStore();const staff=(store.staffMembers||[]).filter(x=>x.active&&!x.deleted&&x.branchId===branchId).map(x=>({id:x.id,userId:x.userId||null,username:x.username||'',employeeCode:x.employeeCode,fullName:x.fullName,role:x.role||'STAFF',areaId:x.areaId||null,areaName:x.areaName||''}));res.json({success:true,data:staff})});
...
router.get('/reports',allowPermission('REPORT.VIEW'),async(req,res)=>{
  const s=await getStore(),range=reportRange(req.query),branchId=req.user.role==='ADMIN'?(req.query.branchId||''):req.user.branchId;
  const shifts=s.shifts.filter(x=>range.inRange(x.shiftDate)&&(!branchId||x.branchId===branchId)&&visibleByScope(req.user,x));
  const ids=new Set(shifts.map(x=>x.id));
  const residents=s.shiftResidents.filter(x=>ids.has(x.shiftId));
  const changes=s.changeLogs.filter(x=>!x.deleted&&ids.has(x.shiftId)).map(withAttention);
  const toilets=s.toiletingLogs.filter(x=>!x.deleted&&ids.has(x.shiftId));
  const allVisible=s.changeLogs.filter(x=>!x.deleted&&(!branchId||x.branchId===branchId)&&visibleByScope(req.user,x)).map(withAttention);
  const outstanding=allVisible.filter(x=>x.attentionLevel&&x.attentionStatus==='OPEN').sort((a,b)=>(a.attentionLevel===b.attentionLevel?String(b.occurredAt||b.createdAt).localeCompare(String(a.occurredAt||a.createdAt)):a.attentionLevel==='RED'?-1:1));
  ...
  res.json({success:true,data:{from:range.from,to:range.to,branchId,shifts:shifts.length,uniqueResidents:new Set(residents.map(x=>x.residentId)).size,changes:changes.length,openRed:outstanding.filter(x=>x.attentionLevel==='RED').length,openYellow:outstanding.filter(x=>x.attentionLevel==='YELLOW').length,resolvedToday:resolvedInRange.length,resolutionRate:attentionInRange.length?Math.round(resolvedInRange.length*100/attentionInRange.length):100,requiresHandover:changes.filter(x=>x.requiresHandover).length,toiletingLogs:toilets.length,toiletingAbnormal:toilets.filter(x=>x.bowelStatus!=='NORMAL'||x.urineStatus!=='NORMAL').length,byCategory,byArea,shiftDetails,residentSummaries,outstanding,details:changes.sort((a,b)=>String(b.occurredAt||b.createdAt).localeCompare(String(a.occurredAt||a.createdAt))),toiletingDetails:toilets.sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)))}});
});
router.get('/reports/staff',allowPermission('REPORT.VIEW'),async(req,res)=>{
  const s=await getStore(),range=reportRange(req.query),branchId=req.user.role==='ADMIN'?(req.query.branchId||''):req.user.branchId;
  const shifts=s.shifts.filter(x=>range.inRange(x.shiftDate)&&(!branchId||x.branchId===branchId)&&visibleByScope(req.user,x));const ids=new Set(shifts.map(x=>x.id));
  const changes=s.changeLogs.filter(x=>!x.deleted&&ids.has(x.shiftId)&&visibleByScope(req.user,x));const handovers=s.handovers.filter(x=>ids.has(x.shiftId));const byId=new Map();
  const calendar=shifts.map(shift=>({id:shift.id,shiftDate:shift.shiftDate,shiftType:shift.shiftType,status:shift.status,branchId:shift.branchId,branchName:shift.branchName||'',areaName:shift.areaName||'Toàn cơ sở',handover:!!handovers.find(h=>h.shiftId===shift.id&&h.confirmedAt),primaryRecorderId:shift.primaryRecorderId||shift.assignedStaffId||'',primaryRecorderName:shift.primaryRecorderName||shift.assignedStaffName||'',staff:(shift.assignedStaff||[]).map(p=>({id:p.id,employeeCode:p.employeeCode||'',fullName:p.fullName||''}))}));
  for(const shift of shifts){for(const person of (shift.assignedStaff||[])){const key=String(person.id||person.employeeCode||person.fullName);const row=byId.get(key)||{id:key,employeeCode:person.employeeCode||'',fullName:person.fullName||'',branchName:shift.branchName||'',shiftCount:0,primaryCount:0,changeCount:0,redCount:0,openCount:0,shiftIds:[],shifts:[]};row.shiftCount++;if(person.id===shift.primaryRecorderId)row.primaryCount++;const authored=changes.filter(x=>x.shiftId===shift.id&&(x.createdBy===person.userId||x.createdBy===person.id));row.changeCount+=authored.length;row.redCount+=authored.filter(x=>attentionLevel(x)==='RED').length;row.openCount+=authored.filter(x=>attentionStatus(x)==='OPEN').length;row.shiftIds.push(shift.id);row.shifts.push({id:shift.id,shiftDate:shift.shiftDate,shiftType:shift.shiftType,status:shift.status,areaName:shift.areaName||'Toàn cơ sở',branchName:shift.branchName||'',handover:!!handovers.find(h=>h.shiftId===shift.id&&h.confirmedAt),staff:(shift.assignedStaff||[]).map(p=>({id:p.id,employeeCode:p.employeeCode||'',fullName:p.fullName||''}))});byId.set(key,row)}}
  res.json({success:true,data:{from:range.from,to:range.to,branchId,calendar,staffDetails:[...byId.values()].sort((a,b)=>String(a.fullName).localeCompare(String(b.fullName),'vi'))}});
});
router.get('/reports/resident/:residentId',allowPermission('REPORT.VIEW'),async(req,res)=>{
  const s=await getStore(),range=reportRange(req.query),residentId=String(req.params.residentId),branchId=req.user.role==='ADMIN'?(req.query.branchId||''):req.user.branchId;
  const shifts=s.shifts.filter(x=>range.inRange(x.shiftDate)&&(!branchId||x.branchId===branchId)&&visibleByScope(req.user,x));const ids=new Set(shifts.map(x=>x.id));
  const roster=s.shiftResidents.filter(x=>ids.has(x.shiftId)&&String(x.residentId)===residentId);const changes=s.changeLogs.filter(x=>!x.deleted&&ids.has(x.shiftId)&&String(x.residentId)===residentId).map(withAttention).sort((a,b)=>String(b.occurredAt||b.createdAt).localeCompare(String(a.occurredAt||a.createdAt)));const toileting=s.toiletingLogs.filter(x=>!x.deleted&&ids.has(x.shiftId)&&String(x.residentId)===residentId).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));const base=roster[0]||changes[0]||toileting[0];if(!base)return res.status(404).json({success:false,message:'Không có dữ liệu NCT trong khoảng đã chọn'});
