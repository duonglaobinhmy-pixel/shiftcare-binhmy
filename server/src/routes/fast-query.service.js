...
export async function getShiftDetailFast(user,id){
  ...
  const changes=cr.rows.map(mapCareRow).map(x=>({...x,eventCodes:eventMap.get(x.id)||[x.eventType],woundImages:imgMap.get(x.id)||[]}));
