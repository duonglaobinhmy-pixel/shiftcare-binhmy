...
const app=express();app.use(cors());app.use(express.json({limit:'8mb'}));app.use(morgan('dev'));
startMediaRetentionCleanup();
app.get('/api/health',async(_req,res)=>res.json({ok:true,service:'shiftcare-bcare-middleware',database:await databaseHealth(),architecture:'BCARE_API_SOURCE_OF_TRUTH_PLUS_LOCAL_CARE_EXTENSIONS',pwa:true,ai:process.env.GEMINI_API_KEY?'GEMINI':'LOCAL_DEMO',telegramAlerts:!!(process.env.TELEGRAM_BOT_TOKEN&&process.env.TELEGRAM_ALERT_CHAT_ID),time:new Date().toISOString()}));
...
const port=Number(process.env.PORT||8788);app.listen(port,'0.0.0.0',()=>console.log(`ShiftCare API listening on :${port}`));
